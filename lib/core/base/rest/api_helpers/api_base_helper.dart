...
class ApiBaseHelper {
  Future<ApiResponse<T>> getGraphql<T extends BaseModel>({
    ...
    try {
      GraphQLClient qlClient = GraphQLClient(
        link: HttpLink(path, defaultHeaders: headers ?? {}),
        cache: GraphQLCache(
          store: HiveStore(),
        ),
      );
  ...
  ApiResponse<T> _returnQueryResult<T extends BaseModel>(
      QueryResult<Object?> queryResult) {
    if (queryResult.data != null) {
      return ApiResponse(BaseModel.fromJson<T>(queryResult.data!), null);
    } else if (queryResult.hasException) {
      var parsedJson = json.decode(queryResult.exception.toString());
      ApiError error;
      if (parsedJson is String) {
        error = ApiError.fromJson(
          {'error_message': parsedJson},
        );
